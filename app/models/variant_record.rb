class VariantRecord
  ...
  def build_records(upload_id)
    ...
    self.samples.each do |sample_name, sample_data|
      ...
      this_subject = Subject.find_by_name(sample_name) || Subject.new(:name => sample_name)

      sample_data.each_pair do |key, value|
      	case key
      			when 'GT'
      			  this_sample.genotype = value
      			when 'DP'
      				this_sample.total_read_depth = value
      			when 'GQ'
      				this_sample.conditional_genotype_quality = value
      			when 'PL'
